...
module.exports = function(options) {
  ...
  this.add('role:provisioner,cmd:startGame',function(msg,respond){
    console.log('----------------Inside Provisionel MS------------------');
    console.log('playersPerGame : ' + playersPerGame);
    console.log('generateQuestions : ' + generateQuestions);
    console.log('questionsPerGame : ' + questionsPerGame);
    console.log('questionTime : ' + questionTime);
    const lobbyId=msg.lobby;

    console.log('Lobby : ' + msg.data.lobby);
    console.log('Players are :-');
    console.log(msg.data.players);

    queue[msg.data.lobby] = [];
    const topicQueue = queue[msg.data.lobby];

    playerCount = 0;
    msg.data.players.map(function(playerRow, i) {
      topicQueue.push(playerRow.name);
      playerCount = i;
    });
    playerCount++;
    
    if(playerCount == playersPerGame - 1)
    {
      createGame(msg.data.lobby, 1);
    }
    else
    {
      createGame(msg.data.lobby);
    }
